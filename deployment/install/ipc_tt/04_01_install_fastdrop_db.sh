...
(
    ...
    CONSTRAINT items_customer_key UNIQUE (time_stamp, identification_code, authorization_code, session_id)
);"
psql -U postgres -d $db -h localhost -c "\d items"
...
psql -U postgres -d $db -h localhost -c "\d items_log"

psql -U postgres -d $db -h localhost -c \
"SELECT create_hypertable('items_log', 'time_stamp', chunk_time_interval => INTERVAL '1 day', if_not_exists => true, migrate_data => true );"

psql -U postgres -d $db -h localhost -c \
"ALTER TABLE items_log SET (
  timescaledb.compress,
  timescaledb.compress_segmentby = 'time_stamp'
);"

psql -U postgres -d $db -h localhost -c "SELECT add_compression_policy('items_log', INTERVAL '7d');"
psql -U postgres -d $db -h localhost -c "SELECT add_retention_policy('items_log', INTERVAL '365d');"

psql -U postgres -d $db -h localhost -c \
"CREATE OR REPLACE FUNCTION i_log_items()
    RETURNS trigger
  AS
\$\$
BEGIN
         INSERT INTO items_log(time_stamp,items_id,session_id,actual_state)
         VALUES(now(), NEW.id,NEW.session_id,NEW.actual_state);
 
    RETURN NEW;
END;
\$\$
LANGUAGE plpgsql;"
# psql -U postgres -d $db -h localhost -c "\df+ i_log_items"

psql -U postgres -d $db -h localhost -c \
"CREATE OR REPLACE TRIGGER i_log_items_trigger
  AFTER INSERT
  ON items
  FOR EACH ROW
  EXECUTE PROCEDURE i_log_items();"

psql -U postgres -d $db -h localhost -c \
"CREATE OR REPLACE FUNCTION u_log_items()
  RETURNS trigger AS
\$\$
BEGIN
         INSERT INTO items_log(time_stamp,items_id,session_id,actual_state)
...
psql -U postgres -d $db -h localhost -c \
"CREATE OR REPLACE TRIGGER u_log_items_trigger
  AFTER UPDATE
  ON items
  FOR EACH ROW
  EXECUTE PROCEDURE u_log_items();"
