...
const Datatypes = {
  ...
  BOOL: {
    bytes: 1,
    parser: (buffer, offset = 0, amount = 1, bit = 0) => + buffer.readUInt8(offset) >> bit & 1 === 1,
    formatter: v => new Buffer.from([v ? 0x01 : 0x00]),
  ...
  CHAR: {
    bytes: 1,
    parser: (buffer, offset = 0, amount = 1) => buffer.toString('ascii', offset, offset + amount),
    formatter: v => new Buffer.from(v, 'ascii'),
